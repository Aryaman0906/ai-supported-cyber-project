...
Use the Cloud Run/Pub/Sub design only after authentication, IAM, OAuth consent, token storage, and report privacy are hardened.
...
- store OAuth and application secrets in Secret Manager;
...
- do not deploy this against private Gmail accounts unless the account owner has explicitly authorized the app.
...
Never commit real credentials, tokens, OAuth client files, encryption keys, Drive links, generated reports, logs, or personal Gmail data.
...
Then grant invoker access only to a specific approved principal, for example your test account or a dedicated service account:
...
The OAuth flow includes `drive.file`. `/reports/generate-daily` can upload Markdown and CSV files to a Drive folder named `AI Cyber Reports` when Drive upload is requested and the user has completed OAuth. If Drive upload is not requested, reports are saved locally under `reports/generated/YYYY-MM-DD/`.
...
- **Drive upload permission issue:** Ensure OAuth completed with `drive.file` scope and Drive folder access is correct.
