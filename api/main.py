...
from api.polling_dashboard import generate_today_report, latest_log, polling_status, reports_index, run_scan_now
...
class PollingScanRequest(BaseModel):
...
def local_polling_scan_now(request: PollingScanRequest | None = None) -> dict[str, Any]:
    ...
    try:
        return run_scan_now(limit=request.limit, dry_run=request.dry_run)
    except Exception as error:
